...
Restoration requires the currently stored room still to be the replacement produced by the matching operation at the expected post-join revision, with matching ID/code/indexes, while the supplied previous room must be that replacement's recorded prior immutable value at the exact preceding revision and pass complete invariants. It restores that exact repository-owned previous object reference without cloning or reconstruction, preserves the room-code index, and consumes the rollback operation record. Reconstructed inputs, completed rollback reuse, newer state, and unrelated state are rejected. Any cleanup failure becomes sanitized `INTERNAL_ERROR`; no success receipt is stored.
